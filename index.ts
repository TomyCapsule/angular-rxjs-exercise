import { Subject, combineLatest, merge, race, interval, concat, zip } from 'rxjs';
import {
  filter,
  distinct,
  skipUntil,
...
cars$.pipe(
  filter(car => ['red','black'].includes(car.color)),
  map(car => car.color),
  distinctUntilChanged(),
  tap(console.log),
)
...
combineLatest(cars$, trucks$).pipe(
  tap(console.log)
)
...
cars$.pipe(
  filter(car => ['Ford', 'Volvo'].includes(car.make)),
  map(car => car.make),
  mergeMap(make => trucks$.pipe(
    filter(truck => truck.make === make)
  ))
)
